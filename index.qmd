...
# Economic Growth and Labor Market Trends: Insights from GDP, Employment, and Consumption

## 1. Labor Force Participation and Economic Growth Over Time
```{r}
ggplot(data_available_clean, aes(x = year, y = lfpr_total_15_64_milo, color = country_name, group = country_name)) +
  ...
  labs(
    ...
    subtitle = "Tracking Workforce Engagement Across Countries",
    x = "Year", 
...
## 2. GDP Growth Trends Over Time
Economic growth, measured by GDP growth rates, reflects a country’s overall economic health. By visualizing GDP growth over time, we can identify economic cycles, including periods of expansion, recession, or stagnation.

```{r}
ggplot(data = data_available_clean, mapping = aes(x = year, y = gdp_growth, fill = country_name)) +
  geom_area(alpha = 0.5, position = "identity") +  # Uses geom_area to fill under the curve
  facet_wrap(~country_name) +
  scale_x_continuous(breaks = seq(min(data_available_clean$year), max(data_available_clean$year), by = 5)) +
  scale_y_continuous(labels = scales::percent) +
  labs(
    title = "GDP Growth Over Time",
    subtitle = "Visualizing Economic Trends Across Countries",
    x = "Year",
    y = "GDP Growth (%)",
    caption = "Source: World Bank Data"
  ) +
  scale_fill_brewer(palette = "Blues", name = "Country") +  # Uses a color palette for clarity
  theme_minimal() +
  theme(
        panel.grid = element_blank(),  
...
##3. Relationship Between Final Consumption Growth and Labor Force Participation
Final consumption, which includes household and government expenditures, is a major driver of GDP. A high labor force participation rate (LFPR) should, in theory, lead to strong consumption growth, as more people in the workforce typically have greater disposable income. However, if LFPR is high but consumption growth is weak, it could indicate low wages, high savings rates, or economic uncertainty.
To analyze this, we plot final consumption growth against LFPR.
```{r}
ggplot(data = data_available_clean, mapping = aes(x = lfpr_total_15_64_milo, y = final_cons_growth, color = country_name, size = gdp_growth)) +
  geom_point(alpha = 0.7) +  # Scatter plot to visualize data points
  geom_smooth(method = "lm", se = FALSE, color = "black", linetype = "dashed") +  # Adds trend line
  facet_wrap(~country_name) +
  scale_x_continuous(labels = scales::percent) +
  scale_y_continuous(labels = scales::percent) +
  labs(
    title = "Final Consumption Growth vs. Labor Force Participation",
    subtitle = "Examining the Link Between Workforce Engagement and Consumer Demand",
...
##4. B
```{r}
ggplot(data = data_available_clean, mapping = aes(x = year, y = self_employed_total, fill = country_name)) +
  geom_col(position = "dodge", alpha=0.8) +  # Bar chart with grouped countries
  scale_x_continuous(breaks = seq(min(data_available_clean$year), max(data_available_clean$year), by = 5)) +
  scale_y_continuous(labels = scales::percent) +
  scale_fill_brewer(palette = "Set2", name = "Country") +  
  labs(
    title = "Self-Employment Trends Over Time",
    subtitle = "Comparing Self-Employment Rates Across Countries",
    x = "Year", 
    y = "Self-Employment as % of Total Employment",
    caption = "Source: World Bank Data"
  ) +
  theme_minimal() +
  theme(
